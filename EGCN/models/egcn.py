...
class egcn(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.opt = opt
        encoder_layers = nn.TransformerEncoderLayer(d_model=300, nhead=2)
    ...
    def forward(self, inputs):
        ...
        text_1 = self.transformer_encoder(text)         #trans
        text = self.text_embed_dropout(text)
        text_1 = self.text_embed_dropout(text_1)        #trans

        text_out, (_, _) = self.text_lstm(text, text_len)
        text_o, (_, _) = self.text_lstm(text_1, text_len) # trans
        ...
        alpha_mat = torch.matmul(x, text_o.transpose(1, 2))   #modify
